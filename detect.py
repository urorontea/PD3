...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # Inference
        with dt[1]:
            #increment_pathは重複しないファイル名を生成する
        ...
        for i, det in enumerate(pred):  # per image
            ...
            #print(im.shape[2:])
            #gnはイメージのサイズを正規化するための係数
            gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh
            #imcは切り取り保存の貯めにim0をコピーしたイメージが格納される
            imc = im0.copy() if save_crop else im0  # for save_crop
            #annotatorは推論結果を可視化するためのなにか
    ...
    if save_txt or save_img:
        ...
        #物体認識で得た結果を別のフォルダに保存する
        #バウンディングボックスの中身だけの画像、ラベルの情報（今のところ、電車or新幹線）
...
def save_frame_and_crop(video_path, frame_number, output_path, x, y, width, height):
    ...
    if success:
        ...
        print(f"フレーム {frame_number} の指定座標内の画像を保存しました: {output_path}")
    else:
        print("指定したフレームが見つかりませんでした")
